...
def prepare_features(df, target_column='glucose_value', forecast_minutes=30):
    ...
    # Calculates rows to shift for forecast
    forecast_steps = int(forecast_minutes / avg_time_diff)
    
    # Creates target, shifts glucose values
    df['future_glucose'] = df[target_column].shift(-forecast_steps)
    
    # Creates binary target for low blood sugar (LBS) prediction
    df['future_hypo'] = (df['future_glucose'] < 70).astype(int)
    
    # Drop rows when there's no future values
    ...
    # Filters columns out that don't exist in DF
...
def train_model(df):
    ...
    # Regression model for glucose prediction
    glucose_pipeline = Pipeline([
        ('scaler', StandardScaler()),
        ('model', RandomForestRegressor(n_estimators=100, random_state=42))
    ])
    
    # Classification model for LBS prediction
